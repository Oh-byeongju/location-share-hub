...
webix.skin.air = {
	topLayout:"wide",
	//bar in accordion
	barHeight:30,			//!!!Set the same in skin.less!!!
	tabbarHeight: 30,
	rowHeight:30,
	toolbarHeight:22,
	...
	layoutMargin:{ space:5, wide:4, clean:0, head:4, line:-1, toolbar:4, form:5  },
	//padding - distance insede cell between cell border and cell content
	layoutPadding:{ space:5, wide:0, clean:0, head:0, line:0, toolbar:4, form:5  },
...
webix.protoUI({
	...
	linkInput: function(input){
		...
		webix.event(node,"keyup",function(e){
			if (node != document.body || this.isVisible())
				this._suggestions(e);
		},this);
	...
	_suggestions: function(e){
		...
		if((e.type == "keydown" && $.inArray(e.keyCode, [13, 38, 40]) == -1) || (e.type == "keyup" && $.inArray(e.keyCode, [13, 38, 40]) != -1)) return;
		var list = this.getList();
...
webix.EditAbility={
	...
	edit:function(id, preserve, show){
		//  KJH_DELETE [S]
		/* beforeEditStart를 editStop 뒤로 보냄
...
webix.editors = {
	...
	"select":{
		...
		render:function(){
			...
			return webix.html.create("div", {
				"class":"webix_dt_editor"
			}, "<select onkeydown=\"if (event.keyCode==13) {this.blur();}\" onchange=\"listener.gridEditor.selectchanged('"+colId+"', $(this));\">"+html+"</select>");
		}
	},
	popup:{
		focus:function(){
...
webix.protoUI({
	...
	_render_initial:function(){
		...
		//webix.html.addStyle("#"+this._top_id +" .webix_cell { height:36px; line-height:38px;" +(this._settings.fixedRowHeight?"":"white-space:normal;")+" }");
		//webix.html.addStyle("#"+this._top_id +" .webix_hcell { height:36px; line-height:38px;}");
		...
		for (var i = yr[0]; i < yr[1]; i++){
			...
			if (item){
				...
				if (item.$height){
					...
					if(config.aplycolor != undefined){
						css += (" ud"+item[config.aplycolor]);
					}
...
webix.ui.datafilter = {
	...
	"masterCheckbox":{
		...
		render:function(master, config){ 
			return "<input class='webix_table_masterCheckBox' type='checkbox' "+(config.checked?"checked='1'":"")+">"; 
		}
	},
	"textFilter":{
...
webix.extend(webix.ui.datatable,{
	...
	getHeaderData:function() {
		var grid = this;
		var cols = [];
		var aCols = grid.config.columns;
		for(var i=0, iLen=aCols.length; i<iLen; i++){
			cols.push("{\"id\":\""+aCols[i].id
					+"\", \"paging\":\""+"Y"
					+"\", \"cryption\":\""+checkEmpty(aCols[i].cryption, "")
					+"\", \"width\":\""+checkEmpty(aCols[i].width, 100)
					+"\", \"align\":\""+((isEmpty(aCols[i].css) || aCols[i].css.indexOf("Left")!=-1)?"left":((aCols[i].css.indexOf("Center")!=-1)?"center":"right"))
					+"\", \"sort\":\""+checkEmpty(aCols[i].sort, "string")
					+"\", \"format\":\""+((typeof aCols[i].format == "function")?aCols[i].format.name:"")
					+"\", \"header\":\""+(aCols[i].header[0].text.indexOf("<") != -1 ? "" : aCols[i].header[0].text)
					+"\", \"footer\":\""+(aCols[i].footer[0].text.indexOf("<") != -1 ? "" : aCols[i].footer[0].text)+"\"}");
		}
		return cols;
	}
});
...
function _get_export_xml(grid, config){
	...
	var xml = '<rows profile="color" without_header="true">';
...
function _get_export_data(grid, scheme, config){
	...
	for (var i = 0; i < data.order.length; i++) {
		...
		for (var j = 0; j < scheme.length; j++){
			...
			var config = grid.getColumnConfig(scheme[j].id);

			xml += '<cell sort="'+ config.sort +'"><![CDATA[' + ((value !== null && value !== webix.undefined) ? value.toString().replace(/<[^>]*>/g,"") : "") + ']]></cell>';
		}
		xml += '</row>';
	}
...
function _send_export(url, xml) {
	//console.log(encodeURI(xml));
	//webix.send(url, { grid_xml : encodeURI(xml) }, null, "_blank");
	//httpRequest(url, { grid_xml : encodeURI(xml) }, null, true);
}
...
webix.protoUI({
	...
	defaults:{
		...
		tagTemplate: function (values) {
			return (values.length ? values.length + " item" : "");
		},
		template:function(obj,common){
	...
	_render_value_block:function(obj, common){
		...
		var input = "<input type='text' class='webix_multicombo_input' style='width: "+Math.min(width,(common._inputWidth||7))+"px;height:"+(height-10)+"px;max-width:"+(width-20)+"px' value='"+(common._inputValue||"")+"'/>";
		var html = "<div class='webix_inp_static' tabindex='0' onclick='' style='line-height:"+height+"px;width: " + width + "px;  text-align: " + inputAlign + ";height:30px' >"+list+input +"</div>";
	...
	_set_inner_size: function(){
		...
		if(popup){
			...
			if(text){
				var height = this._inputHeight - 2 * webix.skin.$active.inputPadding - 12;
...
webix.protoUI({
	...
	_get_value: {
		...
		select: function(el) {
			...
			return (index == -1) ? null : el.options[index].value;
		},
		other: function(el) {
			return el.innerHTML;
		}
